...
struct WeatherContent: View {
    ...
    private var skyGradient: some View {
        TimelineView(.periodic(from: .now, by: 1.0 / 30.0)) { timeline in
            skyBackground(time: timeline.date.timeIntervalSinceReferenceDate)
        }
    }
    
    private func skyBackground(time: TimeInterval) -> some View {
        ...
        .overlay(sunGlow(time: time))
        .overlay(precipitationCanvas(time: time))
        .ignoresSafeArea()
    }
    
    private func sunGlow(time: TimeInterval) -> some View {
        RadialGradient(
    ...
    private func precipitationCanvas(time: TimeInterval) -> some View {
        Canvas { context, size in
            let width = size.width
            let height = size.height
            
            let cloudPositions = [
                CGPoint(x: width * 0.2 + CGFloat(sin(time * 0.04)) * 40.0, y: height * 0.2),
                CGPoint(x: width * 0.85 + CGFloat(cos(time * 0.03)) * 50.0, y: height * 0.35)
            ]
            
            for (i, pos) in cloudPositions.enumerated() {
                let cloudRadius = width * (i == 0 ? 0.5 : 0.4)
                ...
                context.fill(
                    ...
                    with: .radialGradient(cloudGradient, center: pos, startRadius: 0, endRadius: cloudRadius)
                )
            }
            
            let rainCount = 45
            for i in 0..<rainCount {
                let seedX = CGFloat((i * 79 + 17) % 1000) / 1000.0
                let seedY = CGFloat((i * 43 + 13) % 1000) / 1000.0
                let fallSpeed = CGFloat(380.0 + Double((i * 7) % 5) * 45.0)
                let startY = seedY * height + CGFloat(time * Double(fallSpeed)).truncatingRemainder(dividingBy: height)
                let startX = (seedX * width - startY * 0.25).truncatingRemainder(dividingBy: width)
                let boundedX = startX >= 0 ? startX : (width + startX)
                let boundedY = startY.truncatingRemainder(dividingBy: height)
            ...
            ForEach(Array(dailyData.enumerated()), id: \.offset) { i, day in
                HStack {
                    ...
                    GeometryReader { geo in
                        let minTemp: CGFloat = 14.0
                        let maxTemp: CGFloat = 28.0
